...
export default function hasMany(type, options) {
  ...
  (true && !(typeof type === 'string' || typeof type === 'undefined') && Ember.assert('The first argument to DS.hasMany must be a string representing a model type key, not an instance of ' + inspect(type) + '. E.g., to define a relation to the Comment model, use DS.hasMany(\'comment\')', typeof type === 'string' || typeof type === 'undefined'));
  ...
  return computed({
    ...
    set: function set(key, records) {
      ...
      }()) && Ember.assert('All elements of a hasMany relationship must be instances of DS.Model, you passed ' + inspect(records), function () {
        return A(records).every(function (record) {
          return record.hasOwnProperty('_internalModel') === true;
        });
      }()));
